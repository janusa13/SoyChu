    ...
    public function index(Request $request)
    {
        ...
        $productosMasVendidosQuery = DB::table('factura_cliente_productos')
            ...
            ->select(
                ...
                DB::raw('SUM(factura_cliente_productos.cantidad_cj) as total_vendido'),
                DB::raw("DATE_FORMAT(facturas_clientes.fecha, '%d/%m/%Y') as fecha") 
            )
        ...
        $productosIngresadosQuery = DB::table('ingresoproducto')
            ...
            ->select(
                'products.descripcion',
                DB::raw('SUM(ingresoproducto.cantidadCJ) as total_ingresado'),
                DB::raw("DATE_FORMAT(factura.fecha, '%d/%m/%Y') as fecha") 
            )
